/*
   This file was generated automatically by the Mojo IDE version B1.3.6.
   Do not edit this file directly. Instead edit the original Lucid source.
   This is a temporary file and any changes made to it will be destroyed.
*/

module mojo_top_0 (
    input clk,
    input rst_n,
    output reg [7:0] led,
    input cclk,
    output reg spi_miso,
    input spi_ss,
    input spi_mosi,
    input spi_sck,
    output reg [3:0] spi_channel,
    input avr_tx,
    output reg avr_rx,
    input avr_rx_busy,
    output reg [23:0] io_led,
    output reg [7:0] io_seg,
    output reg [3:0] io_sel,
    input [4:0] io_button,
    input [23:0] io_dip,
    output reg a,
    output reg b,
    output reg cin,
    input s,
    input cout
  );
  
  
  
  reg rst;
  
  wire [1-1:0] M_reset_cond_out;
  reg [1-1:0] M_reset_cond_in;
  reset_conditioner_1 reset_cond (
    .clk(clk),
    .in(M_reset_cond_in),
    .out(M_reset_cond_out)
  );
  wire [1-1:0] M_tester_a;
  wire [1-1:0] M_tester_b;
  wire [1-1:0] M_tester_cin;
  wire [8-1:0] M_tester_led_expected;
  wire [8-1:0] M_tester_led_result;
  wire [8-1:0] M_tester_led_case;
  reg [1-1:0] M_tester_s_dip;
  reg [1-1:0] M_tester_cout_dip;
  reg [1-1:0] M_tester_s_circuit;
  reg [1-1:0] M_tester_cout_circuit;
  reg [1-1:0] M_tester_mode_dip;
  reg [1-1:0] M_tester_condition_dip;
  reg [5-1:0] M_tester_io_button;
  test_cases_2 tester (
    .clk(clk),
    .rst(rst),
    .s_dip(M_tester_s_dip),
    .cout_dip(M_tester_cout_dip),
    .s_circuit(M_tester_s_circuit),
    .cout_circuit(M_tester_cout_circuit),
    .mode_dip(M_tester_mode_dip),
    .condition_dip(M_tester_condition_dip),
    .io_button(M_tester_io_button),
    .a(M_tester_a),
    .b(M_tester_b),
    .cin(M_tester_cin),
    .led_expected(M_tester_led_expected),
    .led_result(M_tester_led_result),
    .led_case(M_tester_led_case)
  );
  
  always @* begin
    M_reset_cond_in = ~rst_n;
    rst = M_reset_cond_out;
    led = 8'h00;
    spi_miso = 1'bz;
    spi_channel = 4'bzzzz;
    avr_rx = 1'bz;
    io_led = 24'h000000;
    io_seg = 8'hff;
    io_sel = 4'hf;
    io_led[16+7-:8] = M_tester_led_expected;
    io_led[8+7-:8] = M_tester_led_result;
    io_led[0+7-:8] = M_tester_led_case;
    M_tester_io_button = io_button;
    M_tester_mode_dip = io_dip[16+2+0-:1];
    M_tester_condition_dip = io_dip[16+3+0-:1];
    a = M_tester_a;
    b = M_tester_b;
    cin = M_tester_cin;
    M_tester_s_circuit = s;
    M_tester_cout_circuit = cout;
    M_tester_s_dip = io_dip[16+1+0-:1];
    M_tester_cout_dip = io_dip[16+0+0-:1];
  end
endmodule
